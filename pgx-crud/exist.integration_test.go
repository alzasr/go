...
func (suite *Suite) TestExist() {
	...
	suite.Run("smoke", func() {
		res, err := suite.crud.Exist(suite.ctx, IDFilter{ids: []int64{createdBatch[0].ID, createdBatch[1].ID}})

		suite.Require().NoError(err)
		suite.True(res)
	})

	suite.Run("no_found", func() {
		res, err := suite.crud.Exist(suite.ctx, IDFilter{ids: []int64{-100}})
